...
* **작동 프로세스:**
  1. `locations/global` 경로에 대고 리전 클라이언트를 경유하여 `sandboxEnvironments` 생성을 시도합니다.
  2. 성공할 경우, 발급받은 Sandbox Resource Name을 에이전트의 배포 환경 변수(`SANDBOX_RESOURCE_NAME`)로 주입합니다.
  3. 에이전트 내 `CODE_EXECUTOR_TYPE`을 `"SANDBOX"`로 자동 설정해 격리형 샌드박스 연동 모드로 에이전트를 배포합니다.
  *(단, 프로젝트에 Sandbox API 권한이 없는 경우 터미널에 상세 안내와 함께 스크립트가 안전하게 종료됩니다.)*
