...
## 💡 관리형 샌드박스 활용 추천 프롬프트 (4가지 핵심 Case)

Vertex AI 관리형 샌드박스(`AgentEngineSandboxCodeExecutor`)의 강력한 성능과 고유 기능을 테스트해 보기에 가장 알맞은 특화 프롬프트 예시들입니다.

### Case 1. 보안 격리 및 파일 생성 (Safe File I/O)
* **목표**: 에이전트 환경에 해를 끼치지 않는 완벽히 격리된 환경 내에서의 안전한 로컬 파일 쓰기 연산을 검증합니다.
* **추천 프롬프트**:
  > "pandas를 사용하여 임의의 가상 직원 데이터(이름, 부서, 연봉, 입사일) 5행을 생성하고, 이를 'employees.csv' 파일로 저장한 뒤 표 형식으로 내용을 보여줘."

### Case 2. 다중 대화 상태 유지 (Multi-turn Session State)
* **목표**: 일회성 계산에 그치지 않고, 이전 대화 턴에서 선언 및 계산했던 변수나 메모리 데이터가 그대로 유지되는지 검증합니다.
* **추천 프롬프트**:
  * **질문 1**: `"먼저 파이썬 코드로 x = 10, y = 20을 선언하고 두 값을 더한 결과를 출력해줘."`
  * **질문 2 (연속 질문)**: `"방금 메모리에 선언된 x와 y의 합에 5를 곱한 값을 다시 파이썬 코드로 연산하고 최종 결과를 알려줘."`

### Case 3. 시각화 및 파일 아티팩트 보존 (Visualization & Artifacts)
* **목표**: matplotlib 라이브러리를 통해 시각화 차트 이미지 파일을 생성하고, 이것이 지정한 Cloud Storage(GCS) 버킷에 아티팩트로 장기 보관되는지 검증합니다.
* **추천 프롬프트**:
  > "2020년부터 2024년까지의 주요 빅테크 기업(Google, Apple, Microsoft)의 가상 주가 변동 추이를 pandas와 matplotlib을 사용해 트렌디한 선 그래프 차트로 그려주고, 이를 'stock_trends.png' 파일로 저장한 뒤 결과를 요약해줘."

### Case 4. 무거운 알고리즘 연산 및 비교 (Heavy Computation)
* **목표**: CPU, Memory 하드웨어 사양을 동반하는 복잡한 수치 계산이나 다중 알고리즘의 효율성을 time 모듈 등으로 비교 분석합니다.
* **추천 프롬프트**:
  > "피보나치 수열(Fibonacci Sequence)의 35번째 값을 계산하는 단순 재귀(Recursion) 알고리즘과 메모이제이션(Memoization) 알고리즘의 파이썬 코드를 작성해줘. 그리고 time 모듈을 사용해서 두 방식의 실제 실행 시간 차이를 밀리초(ms) 단위로 비교하고 결과를 마크다운 표로 정리해 알려줘."
